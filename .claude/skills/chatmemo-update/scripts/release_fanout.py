...
读一份结构化双语发版条目（唯一事实源），派生到所有更新日志位置：
  ...
  6. release-notes.txt              —— GitHub Release / Chrome 商店「新功能」文本
...
def default_paths():
    """按脚本位置推断默认路径（skill 在 <web>/.claude/skills/chatmemo-update/scripts/）。"""
    website_root = os.path.abspath(os.path.join(SCRIPT_DIR, "..", "..", "..", ".."))
    ext_root = os.path.abspath(os.path.join(website_root, "..", "..", "开发", "chat-memo_DEV"))
    return {
