标题：Chat Memo 兼容全部浏览器啦！还能同步 Kimi 聊天记忆！v1.1.2 正式版发布

内容：

这次我们带来了重磅更新：
1️⃣ 在全部浏览器，包括 Dia、Arc、Atlas 都能正常使用了
2️⃣ 新增 Kimi 平台支持，能自动保存在 Kimi 里的每一条 AI 消息

——————
🚀 本次更新亮点：
1️⃣ 新增 Kimi 平台支持
现在 Chat Memo 也能自动保存 Kimi 的对话记录了，灵感不再丢失，每一句对话都将被妥善保管。

2️⃣ 浏览器兼容性大优化
无论你是用 Dia 还是夸克，任何浏览器都能流畅打开扩展界面，丝滑无阻。
我们解决了部分浏览器无法正常使用的问题，让体验更加顺畅。

3️⃣ 豆包平台问题修复
修复了豆包平台消息保存失败的问题，确保所有对话都能继续保存。

——————
📥 如何更新？
1️⃣ 如果你已经在使用 Chat Memo，插件会自动更新到最新版本。也
可以手动在浏览器的扩展管理页面中更新

2️⃣ 如果你是新用户：
欢迎访问 chatmemo.ai，或 Chrome 商店搜索「chat memo」，开始构建你的 AI 记忆知识库！

如果无法访问 Chrome 商店，可访问本地安装教程：https://chatmemo.feishu.cn/wiki/Pf0VwsQbkiDuDYkiwiLcZPBPnyf

——————
💬 反馈与共建
Chat Memo 的成长离不开每一位用户的建议。如果你有任何想法或遇到问题，欢迎通过产品内的「联系我们」按钮告诉我们，你的声音很重要！

官方网站：chatmemo.ai